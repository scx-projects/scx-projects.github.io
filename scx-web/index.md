...
SCX Web 不是一个完整的“应用框架”，它主要负责把带有 `@Routes` / `@Route` 注解的类编译成 `Route`。实际启动 HTTP 服务时，通常需要配合 `scx-http-x` 和 `scx-http-routing-x` 使用。典型接入方式是通过 `HttpServer` + `Router` + `new ScxWeb().routes(...)` 的方式接入。
...
完整示例会展示路径参数、查询参数、请求体、Multipart、普通对象返回、HTML、文件下载和重定向等用法。
...
从 `multipart/form-data` 请求体中按名称读取 part。当前支持绑定到：
...
`@Part` 默认使用参数名作为 part 名称，`required` 默认为 `true`。如果绑定到单个 `MultiPartPart` 但实际匹配到多个 part，会抛出参数转换异常。绑定前会读取并在内存中缓存全部 multipart part；大文件或需要流式处理时，应直接接收 `ScxHttpServerRequest`。
...
在路由方法及其调用链中，也可以通过 `ScxWeb.routingContext()` 获取当前 `RoutingContext`：

```java
var context = ScxWeb.routingContext();
```

该方法只在 SCX Web 路由执行期间可用。
...
`Json.of(...)` 会设置 `application/json; charset=utf-8` 并使用 SCX Serialize 转 JSON。默认使用当前 `ScxWeb` 的序列化配置，也可以通过 `Json.of(data, serializeConfig)` 为单次响应指定配置。
...
`Xml.of(...)` 会设置 `application/xml; charset=utf-8` 并使用 SCX Serialize 转 XML。默认使用当前 `ScxWeb` 的序列化配置，也可以通过 `Xml.of(data, serializeConfig)` 为单次响应指定配置。
...
当路由方法需要返回一个值，但不希望 SCX Web 对响应执行任何操作时，可以返回 `Noop.NOOP`。

例如，一个路由方法可以在条件匹配时直接返回内容；条件不匹配时调用 `RoutingContext.next()` 把请求交给下一个路由，并返回 `NOOP`：
...
`Noop` 本身不会发送或修改响应，也不会调用 `RoutingContext.next()`；它只表示当前 `WebResult` 不执行任何响应操作。

## 序列化配置

参数转换、普通对象返回以及未单独传入配置的 `Json` / `Xml` 都使用 `ScxWeb` 的 `SerializeConfig`。

```java
import dev.scx.serialize.SerializeConfig;

var scxWeb = new ScxWeb()
    .serializeConfig(SerializeConfig.of().prettyPrint(true));
```

`serializeConfig()` 可以读取当前配置；`serializeConfig(config)` 设置配置时会复制传入的配置。
...
默认错误处理器会根据客户端 `Accept` 返回 HTML 或 JSON 错误信息；如果异常实现了 `ScxHttpException`，会使用异常自身的 HTTP 状态码，否则默认是 500。`DEFAULT_WEB_ERROR_HANDLER` 等价于 `new DefaultWebErrorHandler(true)`，会把堆栈信息写入响应；不希望向客户端暴露堆栈时可以使用 `new DefaultWebErrorHandler(false)`。
...
public class WebSocketController {
    ...
    public void chat(ScxWebSocketServerHandshakeRequest request,
...
public class DemoController {
    ...
    public Object search(@QueryParam String keyword,
                         @QueryParam(required = false) Integer page) {
        return Map.of("keyword", keyword, "page", page == null ? 1 : page);
    }
