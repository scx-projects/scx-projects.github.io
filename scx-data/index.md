...
`FieldPolicy` 描述字段包含、字段排除、空值处理、虚拟字段和字段表达式。它不要求底层个定有“表字段”或“列”的概念，只要求实现者能理解这些字段选择和字段表达式。
...
当普通结构化条件无法描述某个实现特有能力时，可以使用 `whereClause(expression, params...)`。这里的 `expression` 和 `params` 只是个段实现相关描述，具体如何解释由底层 `Repository` 实现决定。
