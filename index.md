...
- [SCX App](./scx-app/index.md)：应用编排内核，负责模块定义、候选类汇总、模块依赖与启动顺序，以及启动/停止流程。
- [SCX App X](./scx-app-x/index.md)：在 SCX App 之上提供配置环境、DI，以及 Web、HTTP、SQL、定时任务、日志、静态资源、自动扫描等常用应用模块。
- [SCX Parent](./scx-parent/index.md)：SCX 项目的 Maven 父工程，统一 Java 版本、插件、发布、测试和格式化配置。
- [SCX DI](./scx-di/index.md)：依赖注入核心，负责显式注册组件、构造函数注入、字段注入、值注入和循环依赖检测。
...
- [SCX Timer](./scx-timer/index.md)：延迟任务抽象，支持任务状态查询和取消。
