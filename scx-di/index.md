...
如果发布 `1.0.0`，将 `version` 替换为对应版本即可。
...
var container = ComponentContainer.builder()
    ...
    .registerComponentClass("userRepository", UserRepository.class)
    .registerComponentClass("userService", UserService.class)
    .build();
...
```java
containerBuilder.registerComponentClass("userService", UserService.class);
```

默认等价于：

```java
containerBuilder.registerComponentClass(
    "userService",
    UserService.class,
    true,
    true
...
```java
containerBuilder.registerComponentClass("task", Task.class, false);
```
...
containerBuilder.registerComponentClass("userService", OtherService.class);
```
...
> 注意：当前源码中 `@Inject` 的 `value` 类型是 `String[]`。实际解析时只使用第一个值。因此推荐只写一个名称，例如 `@Inject("primaryRepository")`。
...
当前源码中，`ValueResolver` 是 `ValueAnnotationDependencyResolver` 的内部接口：
...
```java
containerBuilder.registerComponentClass("a", A.class, false);
containerBuilder.registerComponentClass("b", B.class, false);
```
...
public class DIExample {
    ...
    public static void main(String[] args) throws Exception {
        var container = ComponentContainer.builder()
            ...
            .addDependencyResolver(new InjectAnnotationDependencyResolver())
            .registerComponentClass("config", AppConfig.class)
            .registerComponentClass("userRepository", UserRepository.class)
            .registerComponentClass("userService", UserService.class)
            .build();
...
```java
containerBuilder.registerComponentClass("name", ComponentClass.class);
...
ComponentContainer container = ComponentContainer.builder()
    .addDependencyResolver(new InjectAnnotationDependencyResolver())
    .registerComponentClass("name", ComponentClass.class)
    .build();
```
