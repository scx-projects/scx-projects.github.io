...
SCX JDBC Spy 是一个 JDBC 监听和 SQL 日志工具库。
...
对应值会被格式化为占位符，例如 `<byte[1024]>`、`<InputStream>`、`<Blob>`。

这可以避免在日志中输出大量二进制内容或不可重复读取的流，同时仍然保留参数类型和必要的长度信息。
...
如果 batch 为空，日志会明确显示 `0 batch entries`。
...
`byte[]` 会显示为 `<byte[n]>`，其中 `n` 是字节数组长度。

`InputStream`、`Reader`、`Blob`、`Clob` 等类型会显示为对应占位符，例如 `<InputStream>`、`<Reader>`、`<Blob>`、`<Clob>`。
